...
import { distanceInMetres, type Coordinates } from '../services/maps';
...
export function DriveTaskScreen({ task, acceptedAt, user, onBack, onCompleted }: { task: QuestSubtask; acceptedAt: string; user: User; onBack: () => void; onCompleted: () => void }) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        if (!Number.isFinite(task.destinationLatitude) || !Number.isFinite(task.destinationLongitude)) throw new Error('This drive task has no pinned destination. Edit and republish it with a map pin.');
        const target = { latitude: task.destinationLatitude!, longitude: task.destinationLongitude! };
        if (cancelled) return;
  ...
  }, [db, task.destinationLatitude, task.destinationLongitude, task.id, user.id, onCompleted]);

  const openDirections = () => Linking.openURL(`https://www.google.com/maps/dir/?api=1&destination=${task.destinationLatitude},${task.destinationLongitude}`);
...
const styles = StyleSheet.create({
  ...
  panel: { padding: 18, paddingBottom: 10, borderTopWidth: 1, borderTopColor: theme.colors.border, backgroundColor: theme.colors.white }, livePill: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 9, paddingVertical: 5, borderRadius: 99, backgroundColor: '#FFF3BF' }, dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: theme.colors.orange }, liveText: { color: theme.colors.ink, fontSize: 8, fontWeight: '900', letterSpacing: .7, fontVariant: ['tabular-nums'] }, title: { marginTop: 11, color: theme.colors.ink, fontSize: 22, lineHeight: 26, fontWeight: '800' }, destination: { marginTop: 4, color: theme.colors.muted, fontSize: 11 },
  tripRow: { marginTop: 15, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', gap: 12 }, metricLabel: { color: theme.colors.muted, fontSize: 8, fontWeight: '800', letterSpacing: .8 }, metric: { marginTop: 2, color: theme.colors.orange, fontSize: 24, fontWeight: '900' }, status: { flex: 1, color: theme.colors.muted, fontSize: 10, textAlign: 'right' }, error: { marginTop: 10, color: theme.colors.error, fontSize: 11, fontWeight: '800' }, directions: { minHeight: 48, marginTop: 14, borderRadius: 24, paddingHorizontal: 17, backgroundColor: theme.colors.orange, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, directionsText: { color: theme.colors.white, fontSize: 11, fontWeight: '800' }, directionsArrow: { color: theme.colors.white, fontSize: 18 }, note: { marginTop: 9, color: theme.colors.muted, fontSize: 9, lineHeight: 13, textAlign: 'center' },
});
