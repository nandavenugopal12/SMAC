...
export function FamilyScreen({ user, onFamilyReady, onLogout }: { user: User; onFamilyReady: () => Promise<void>; onLogout: () => Promise<void> }) {
  ...
  return <SafeAreaView style={styles.safe} edges={['top', 'bottom']}><ScrollView contentContainerStyle={[styles.page, compact && compactStyles.page, modern.page]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentInsetAdjustmentBehavior="automatic">
    <View style={styles.top}><View style={styles.identity}><Image accessible accessibilityLabel="Nayah" source={require('../assets/nayah-logo.png')} resizeMode="contain" style={styles.brandLogo} /><Text numberOfLines={1} style={styles.user}>{user.name} · {user.roles.filter(role => role !== 'anyone').join(' · ') || 'Helper'}</Text></View><Pressable hitSlop={10} onPress={onLogout}><Text style={styles.logout}>Log out</Text></Pressable></View>
    <View style={[styles.heading, compact && compactStyles.heading, modern.heading]}><Text style={[styles.eyebrow, modern.eyebrow]}>ONE LAST STEP</Text><Text style={[styles.title, compact && compactStyles.title, modern.title]}>Find your <Text style={styles.orange}>family crew.</Text></Text><Text style={[styles.subtitle, modern.subtitle]}>Hey {user.name.split(' ')[0]} — create a family or enter an existing crew code.</Text></View>
    <OffsetCard style={styles.cardWrap} contentStyle={[styles.card, compact && compactStyles.card, modern.card]} offset={offset}>
    ...
    <OffsetCard style={styles.cardWrap} contentStyle={[styles.card, styles.dark, compact && compactStyles.card, modern.card]} shadowColor={theme.colors.orange} offset={offset}>
...
function Action({ title, busy, light, onPress }: { title: string; busy: boolean; light?: boolean; onPress: () => void }) {
  return <Pressable disabled={busy} onPress={onPress} style={[styles.button, modern.button, light && styles.lightButton]}>{busy ? <ActivityIndicator color={light ? theme.colors.ink : 'white'} /> : <><Text style={[styles.buttonText, light && styles.darkText]}>{title}</Text><View style={styles.arrow}><Text>↗</Text></View></>}</Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  brandLogo: { width: 102, height: 30 }, user: { fontSize: 10, textTransform: 'capitalize', color: theme.colors.muted, marginTop: 3 }, logout: { fontSize: 12, fontWeight: '800', color: theme.colors.ink },
...
const modern = StyleSheet.create({ page: { paddingHorizontal: 18 }, heading: { alignItems: 'flex-start', paddingVertical: 30 }, eyebrow: { borderWidth: 0, paddingHorizontal: 0, paddingVertical: 0, color: theme.colors.muted }, title: { textAlign: 'left', fontSize: 38, lineHeight: 42, letterSpacing: -1.4 }, subtitle: { textAlign: 'left', fontSize: 14, lineHeight: 20 }, card: { borderWidth: 1, borderColor: theme.colors.border, borderRadius: 24 }, button: { backgroundColor: theme.colors.orange } });
