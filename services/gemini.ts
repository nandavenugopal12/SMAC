...
export async function breakDownChore(chore: string): Promise<QuestPlan> {
  ...
  return {
    ...
    tasks: plan.tasks.slice(0, 8).map((task: any, index: number) => ({
      ...
      skill: validRoles.includes(task.skill) ? task.skill : 'anyone', doTogether: Boolean(task.doTogether), destination: String(task.destination || ''),
    })),
  };
}
